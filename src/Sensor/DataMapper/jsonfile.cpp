...
JsonFile::JsonFile(std::string path, Converter::Json& converter): path(std::move(path)), converter(converter) {}
JsonFile JsonFile::fromPath(const std::string& path){
    Converter::Reader reader;
    Converter::Json convert(reader);
    JsonFile data_mapper(path, convert);
    return data_mapper;
}
...
const Converter::Json& JsonFile::getConverter() const{
    return converter;
}
